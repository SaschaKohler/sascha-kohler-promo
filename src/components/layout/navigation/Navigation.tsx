  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-white bg-opacity-95 shadow-sm z-40 transition-all duration-300">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        <div className="text-2xl font-semibold" style={{ color: colorScheme.primary }}>
          <span style={{ color: colorScheme.accent }}>Sascha</span> Kohler
        </div>

        {/* Desktop Menu */}
        <div className="hidden md:flex space-x-8">
          {['Start', 'Über mich', 'Werte', 'Expertise', 'Kontakt'].map((item, index) => (
            <button
              key={index}
              className="transition-all duration-300"
              style={{
                color:
                  activeSection === ['hero', 'about', 'values', 'expertise', 'contact'][index]
                    ? colorScheme.accent
                    : '#666',
                fontWeight:
                  activeSection === ['hero', 'about', 'values', 'expertise', 'contact'][index]
              ...
              onClick={() =>
                scrollToSection(['hero', 'about', 'values', 'expertise', 'contact'][index])
              }
            >
              {item}
            </button>
